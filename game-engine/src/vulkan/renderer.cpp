...
const char *extensions[] = {WINDOW_LIB_EXTENTIONS, "VK_EXT_debug_utils"};
...
int check_layers(const char **validationLayers, int validationLayerCount) {
  uint32_t layerCount;
...
void createInstance(VkInstance *instance,
  ...
  if (!check_extensions(extensions, STATIC_ARR_LEN(extensions))) {
    THROW_EXEPTION("Extention requested but not available!");
  }
  ...
  VkInstanceCreateInfo createInfo =
      ...
      {.sType = VK_STRUCTURE_TYPE_INSTANCE_CREATE_INFO,
       ...
       .enabledExtensionCount = STATIC_ARR_LEN(extensions),
       .ppEnabledExtensionNames = extensions};

  if (enableValidationLayers && !check_layers(validationLayers, layerCount)) {
    THROW_EXEPTION("Validation layers requested, but not available!");
  }
  ...
  if (vkCreateInstance(&createInfo, 0, instance) != VK_SUCCESS) {
    THROW_EXEPTION("Vulkan create info failed");
  }

  // Create debug messenger
  if (CreateDebugUtilsMessengerEXT(*instance, &debugMessangerCreateInfo, 0,
                                   debugMessenger) != VK_SUCCESS) {
    THROW_EXEPTION("failed to set up debug messenger!");
  }
}
...
bool isDeviceSuitable(VkPhysicalDevice physicalDevice, VkSurfaceKHR surface,
  ...
  if (extensionsSupported) {
    swapChainAdequate =
        !(capabilities.chainSupInf.formats.list == nullptr) &&
        !(capabilities.chainSupInf.presentModes.list == nullptr);
  }

  return capabilities.indices.isComplete() && extensionsSupported &&
         swapChainAdequate;
}
...
void createLogicalDevice(VkPhysicalDevice physicalDevice,
  ...
  if (vkCreateDevice(physicalDevice, &createInfo, 0, &deviceP) != VK_SUCCESS) {
    THROW_EXEPTION("Failed to create physical device");
  }
...
void Renderer::_createCommandPool() {
  ...
  if (vkCreateCommandPool(_v.device.get(), &poolInfo, nullptr, &_v.cmdPool) !=
      VK_SUCCESS) {
    THROW_EXEPTION("failed to create command pool!");
  }
}
...
void Renderer::_createCommandBuffers() {
  ...
  if (vkAllocateCommandBuffers(_v.device.get(), &allocInfo, _v.cmdBufs.get()) !=
      VK_SUCCESS) {
    THROW_EXEPTION("failed to allocate command buffers!");
  }
}
void Renderer::_createSyncObjects() {
  ...
  for (int i = 0; i < _framesInFlight; i++) {
    if (vkCreateSemaphore(_v.device.get(), &semaphoreInfo, nullptr,
      ...
      THROW_EXEPTION("failed to create semaphores!");
    }
  }
}
...
void Renderer::createSwapchainFramebufs(window::WindowDims dims,
  ...
  for (size_t i = 0; i < _swapchain->getImageCount(); i++) {
    ...
    if (vkCreateFramebuffer(_v.device.get(), &framebufferInfo, nullptr,
      ...
      throw utils::EngineExeption("failed to create framebuffer!");
    }
  }
}
...
void Swapchain::present(VkSemaphore imgAvailableSem) {
  ...
  if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR) {
    ...
    THROW_EXEPTION("Failed to present image");
  }
}
...
Renderer::Renderer(const window::Window &window) : _window(window) {
  ...
  if (deviceCount == 0) {
    THROW_EXEPTION("No physicalDevices with vulkan support!");
  }
  ...
  if (_v.physicalDevice == VK_NULL_HANDLE) {
    THROW_EXEPTION("failed to find a suitable GPU!");
  }
...
VkCommandBuffer Renderer::beginFrame() {
  ...
  if (vkBeginCommandBuffer(_v.cmdBufs.get()[_currentFrameIndex], &beginInfo) !=
      VK_SUCCESS) {
    THROW_EXEPTION("failed to begin recording command buffer!");
  }
...
void Renderer::endFrame() {
  ...
  if (vkEndCommandBuffer(_v.cmdBufs[_currentFrameIndex]) != VK_SUCCESS) {
    THROW_EXEPTION("failed to record command buffer!");
  }
  ...
  if (vkQueueSubmit(_graphicsQueue, 1, &submitInfo,
    ...
    THROW_EXEPTION("failed to submit draw command buffer!");
  }
